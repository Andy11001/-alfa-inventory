...
- Lista produktów: WordPress JSON API (sklep.peugeot.pl)
- Raty + miasto dealera: bezpośrednie API kalkulatora SFS (sfs_calculator),
  bez Selenium — patrz scrapers/sfs_calculator.py
- Wyjście: peugeot_osobowe_inventory.csv + peugeot_lcv_inventory.csv
"""
import re
import os
import sys
...
# --- Slug kategorii (lub numeryczne ID termu WP) -> nazwa modelu ---
MODEL_MAP = {
    "208": "208",
    "2008": "2008",
    "308": "308",
    ...
    # Numeryczne ID termów WP (z taksonomii product_cat)
    "190": "208",
    "166": "2008",
    "172": "308",
    "175": "308 SW",
...
def get_model_slug(product):
    slug = wp_shop.get_model_slug(product, skip_slugs=("bez-kategorii", "_mto", "456"))
    if slug:
        return slug
    link_match = re.search(r'/produkt/([^/]+)/', product.get("link") or "")
    if link_match and link_match.group(1) not in ["bez-kategorii"]:
...
def build_row(product, rate_info):
    ...
    if model in ["_MTO", "Bez Kategorii"]:
        return None
    ...
    for cls in class_values:
        if cls.startswith("pa_kolor-"):
            ...
            # Wartość bywa rokiem albo ID termu WP
    ...
    tiktok_desc = scraper_utils.format_inventory_description(
        "Peugeot", model, trim, clean_installment, detected_city)
...
def main():
    print("=" * 60)
    print("Peugeot Inventory Feed — sklep.peugeot.pl (API SFS, bez Selenium)")
    print("=" * 60)
    ...
    print(f"\n  Po odfiltrowaniu duchów: {len(all_products)} produktów.")

    print("\n[2/3] Raty + lokalizacje przez API SFS...\n")
    links = [p["link"] for p in all_products]
    rates, stats = sfs_calculator.get_inventory_rates("peugeot", links)
